...
#include "./HmetisHypergraphPartitioning.hpp"

namespace Hydra::HypergraphPartitioning::Hmetis {

    template <typename VarT, typename LiteralT, typename ClauseIdT>
    bool HmetisHypergraphPartitioning<VarT, LiteralT, ClauseIdT>::waitUntilFileExistsOrTimeout(const std::string& fileName) const {
        int counter = 0;
    ...
    template <typename VarT, typename LiteralT, typename ClauseIdT>
    void HmetisHypergraphPartitioning<VarT, LiteralT, ClauseIdT>::createHypergraphFile(const OccurrenceListStruct& occurrenceListStruct) {
        // Fill xpins and pins
    ...
    template <typename VarT, typename LiteralT, typename ClauseIdT>
    void HmetisHypergraphPartitioning<VarT, LiteralT, ClauseIdT>::createFixFile(const OccurrenceListStruct& occurrenceListStruct,
    ...
    template <typename VarT, typename LiteralT, typename ClauseIdT>
    void HmetisHypergraphPartitioning<VarT, LiteralT, ClauseIdT>::parsePartitionFile(ClauseIdT numberOfVertices) {
        // File does not exist
...

        {
            ...
            while (getline(fileStream, line)) {
                try {
                    ...
                    throw Exception::HypergraphPartitioning::InvalidParsedPartitionHmetisException(line);
                }
            }
        }
    ...
    template <typename VarT, typename LiteralT, typename ClauseIdT>
    typename HmetisHypergraphPartitioning<VarT, LiteralT, ClauseIdT>::VariableSetType
    HmetisHypergraphPartitioning<VarT, LiteralT, ClauseIdT>::computeCut(const OccurrenceListStruct& occurrenceListStruct) {
        ...
        if (!waitUntilFileExistsOrTimeout(hypergraphFileName_))
            throw Exception::HypergraphPartitioning::FileContainingSomethingCannotBeFoundHmetisException(hypergraphFileName_, "hypergraph");
        // if (!waitUntilFileExistsOrTimeout(fixFileName_))
        //     throw Exception::HypergraphPartitioning::FileContainingSomethingCannotBeFoundHmetisException(fixFileName_, "pre-assignment");
    ...
    #ifndef NDEBUG
    template <typename VarT, typename LiteralT, typename ClauseIdT>
    void HmetisHypergraphPartitioning<VarT, LiteralT, ClauseIdT>::processPrintHypergraphPartitioningDebug(std::ostream& out) const {
